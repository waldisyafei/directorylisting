...
@section('content')
	...
	<div class="container-fluid">
		<div class="row">
			...
			<form method="post" action="{{ url('account/listings/edit', $listing->id) }}" class="form-horizontal" enctype="multipart/form-data">
									...
									<select class="form-control" name="category">
										...
										@foreach ($categories as $category)
											@if ($category->parent == 0)
												<option value="{{ $category->id }}"{{ $listing->listingCategory->parent == $category->id ? ' selected' : null }}>{{ $category->title }}</option>
											@endif
										@endforeach
									</select>
								</div>
