					...
					<form class="form-horizontal row-border" method="post" action="{{ url('app-admin/ads/create') }}">
						...
						<div class="panel-body" style="padding: 40px 16px;">
							<div class="form-group">
								...
								<div class="col-sm-8">
									...
									<select class="form-control" name="customer_id" id="customer-id">
										...
										<option value="non-customer"{{ (old('customer_id') == 'non-customer' ? ' selected' : null) }}>Non Customer</option>
										@foreach ($customers as $customer)
											<option value="{{ $customer->id }}"{{ (old('customer_id') == $customer->customer_id ? ' selected' : null) }}>{{ $customer->customer_name }}</option>
										@endforeach
									</select>
								</div>
							</div>
							...
							<div class="form-group" id="set-addpass" style="display: none;">
								...
								<div class="col-sm-8">
									<input type="text" name="add_password" placeholder="Ad Password" class="form-control">
								</div>
							</div>
							...
							<div class="form-group">
								...
								<div class="col-sm-8">
									<input type="text" name="days_to_show" value="1" class="form-control">
								</div>
							</div>
							<div class="form-group">
